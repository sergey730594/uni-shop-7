...
// Иконки меню — красные контурные
const HomeIcon = () => (
  <svg className="w-4 h-4 text-[#ff0000]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
...
export const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, onClose, language }) => {
  ...
  const categories = cakeCategories[language as keyof typeof cakeCategories] || cakeCategories.ka;
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 h-full w-80 bg-white shadow-2xl z-50 lg:hidden flex flex-col animate-slide-in-left">
          ...
          {items.map((item, index) => {
              ...
              return (
                <div key={index}>
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-3 px-6 py-3.5 border-b border-gray-50 text-gray-700 hover:bg-red-50"
                  >
                  ...
                  {isCakesOpen && (
                    <div className="bg-gray-50 py-1 max-h-60 overflow-y-auto">
                      {categories.map((cat, catIndex) => (
                        <Link
                          key={catIndex}
                          to={`/${language}/cakes/${cat.slug}`}
                          onClick={onClose}
                          className="flex items-center gap-2 px-8 py-2 text-xs font-medium text-gray-600 hover:text-[#ff0000] hover:bg-red-50"
                        >
            ...
            return (
              <Link
                ...
                onClick={onClose}
                className="flex items-center gap-3 px-6 py-3.5 border-b border-gray-50 text-gray-700 hover:bg-red-50"
              >
                {menuIcons[item.icon]}
                <span className="font-medium text-sm">{item.name}</span>
              </Link>
        ...
        <div className="border-t border-gray-200 p-4 flex-shrink-0">
          <p className="text-xs text-gray-500 text-center">© 2026 Grant Bakery</p>
        </div>
      </div>
    </>
  );
