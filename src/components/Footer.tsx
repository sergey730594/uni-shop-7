...
// ==================== ИКОНКИ СОЦСЕТЕЙ (ВСЕ ОДИНАКОВОГО РАЗМЕРА) ====================
const SocialIcon = ({ children, color }: { children: React.ReactNode; color: string }) => (
  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${color} text-white hover:scale-110 transform transition duration-200 shadow-md hover:shadow-lg`}>
    {children}
  </div>
);
    ...
    <footer className="bg-white border-t border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
          ...
          {/* ====== СРЕДНЯЯ КОЛОНКА - ИКОНКИ СОЦСЕТЕЙ В РЯД ====== */}
          ...
          <div className="space-y-3">
            <h3 className="font-semibold text-gray-800 text-lg text-center md:text-left">
              {language === 'ka' ? 'ჩვენს მისამართზე' :
               language === 'en' ? 'Our location' :
               language === 'ru' ? 'Мы на карте' :
               'Konumumuz'}
            </h3>
